...
int main(int argc, char** argv)
{
	...
	while(head!=NULL)
	{
		if(flag>9)
		{
            ...
            else if(head->getstat()==0)//if ready
            {
                if (!is_run)
                {
                    ...
                    {
						fout << endl << "==> " << head->getnt() << " " << head->getpid() << " ts=" << head->getnt()-head->getdur() << " RUNNG  dur=" << head->getdur() << endl;
		...
		{
			if(head->getstat()==0 && !is_run && (flag%10!=4 || head->getbt()==0))//if ready
            {
				randfile >> rand;
                randnum--;
                head->gcb(rand);
            ...
            else if(head->getstat()==1)//if run
            {
				...
				if ((flag%10!=4 || head->getbt()<=r_num) && head->getrem()!=0)
                {
					randfile >> rand;
					randnum--;
                    head->gib(rand);
